...
public class ServerCore {
	...
	//定义一个日期格式
	static final SimpleDateFormat dateFormat = new SimpleDateFormat("YYYY-MM-dd HH:mm:ss", java.util.Locale.SIMPLIFIED_CHINESE);
	...
	static void removeConnection (SelectionKey clientKey) {
		...
		if (!Server.clients.contains(clientKey)) return;
	...
	public static void config() {
		...
		for(String s : Server._args) {
			if(s.contains("-address:")) {
				...
				if (addressTemp.length() > 0) address = addressTemp;
			}else if(s.contains("-port:")) {
				String portTemp = s.split(":", 2)[1];
				if (portTemp.length() > 0) port = Integer.valueOf(portTemp);
			}else if(s.contains("-password:")) {
				String pwd = s.split(":", 2)[1];
				if(pwd.length() > 0) Server.pwd = pwd;
			}
		}
		if(address == null) {
